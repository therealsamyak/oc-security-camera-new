...
from config_loader import BatchConfig
...
class BatchSimulationRunner(SimulationRunnerBase):
    ...
    def run_batch_simulations(self, parallel: bool = True) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Starting batch simulation with {len(variations)} parameter variations")
            self.logger.info(f"Each variation will run {len(base_simulations)} simulations")
            self.logger.info(f"Total simulations to run: {len(variations) * len(base_simulations)}")

            successful_results = []

            # Run simulations for each parameter variation
            for variation in variations:
                self.logger.info(f"Running variation {variation['variation_id']}/{len(variations)}")
            ...
            if len(self.failed_simulations) > 0:
                ...
                self.logger.error("TERMINATING DUE TO FAILURES - NO OUTPUT WILL BE GENERATED")
                
                # Print clear error information
                for failure in self.failed_simulations[:5]:  # Show first 5 failures
                    self.logger.error(f"FAILED: {failure.get('simulation_id', 'unknown')} - {failure.get('error', 'unknown error')}")
                
                if len(self.failed_simulations) > 5:
                    self.logger.error(f"... and {len(self.failed_simulations) - 5} more failures")
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_sim):
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Simulation {sim['simulation_id']} (variation {variation['variation_id']}) raised exception: {e}"
                        )
    ...
    def _export_batch_results(self):
        ...
        try:
            ...
            metadata = {
                ...
                "total_variations": len(set(r["variation_id"] for r in self.all_results)),
