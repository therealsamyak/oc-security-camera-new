...
def test_basic_functionality():
    ...
    try:
        ...
        print(f"   Config: {config.duration_days} days, {config.task_interval_seconds}s interval")
        ...
        print(f"   Battery: {battery.get_level_wh():.2f}Wh / {battery.capacity_wh:.2f}Wh")
        print(f"   Percentage: {battery.get_percentage():.1f}%")
        
        # Test 4: Test Controllers
        print("4. Testing Controllers...")
        from controller import NaiveWeakController, NaiveStrongController, CustomController
        ...
        weak_choice = weak_controller.select_model(
            battery_level=50.0, clean_energy_percentage=30.0,
            user_accuracy_requirement=80.0, user_latency_requirement=2000.0,
        ...
        if task:
            print(f"   Generated task: accuracy={task.accuracy_requirement:.1f}%, latency={task.latency_requirement:.0f}ms")
        else:
        ...
        print(f"   Loaded config: {sim_config.duration_days} days, locations: {sim_config.locations}")
        ...
        print(f"   Metrics: {metrics['total_tasks']} tasks, {metrics['task_completion_rate']:.1f}% completion")
...
def test_short_simulation():
    ...
    try:
        ...
        print(f"   Energy used: {metrics.get('total_energy_wh', 0):.4f} Wh")
        print(f"   Clean energy: {metrics.get('clean_energy_percentage', 0):.1f}%")
