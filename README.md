...
THE GOAL OF THIS PAPER IS TO OBTAIN A MODEL WITH GOOD ENOUGH WEIGHTS SO IT CAN BE RAN GENERAL-USE IN ALL SCENARIOS TO OPTIMIZE THIS.
...
Once we have information, we can assign a global 'battery level', and assign costs to each model based on the power usage we found.
...
We will run the simulation for '24 hours', with a task call every 5 second. Note that we get a new clean energy datapoint every 5 minutes, so keep using the latest available clean energy datapoint that is not in the future until 5 minutes have passed, then use the new one.

We will run the simulation for 4 days, one from every season.
...
- **ORACLE (OMNISCIENT)**: Takes in all problem information (historical clean energy data, accuracy, latency requirements), and uses a Mixed-Integer Linear Programming (MILP) solver (python has many pre-existing libraries that do this) to maximize the amount of clean energy used ONLY. Due to the nature of MILP solvers, optimizing for a proportion won’t work, hence why we are optimizing for amount of clean energy consumed instead. Because this is the oracle controller, it can see the future and has full system knowledge of the clean energy information of that day, hence why an MILP solver is appropriate.
