...
class TestBatteryIntegration(unittest.TestCase):
    ...
    def test_energy_calculation_accuracy(self):
        ...
        test_cases = [
            {'power_mw': 1000, 'duration': 3600, 'expected_wh': 1.0},    # 1W for 1 hour = 1Wh
            {'power_mw': 2000, 'duration': 1800, 'expected_wh': 1.0},    # 2W for 30 min = 1Wh
            {'power_mw': 500, 'duration': 7200, 'expected_wh': 1.0},     # 0.5W for 2 hours = 1Wh
            {'power_mw': 3476.75, 'duration': 1, 'expected_wh': 0.000965764},  # YOLOv10_X for 1 second
        ]
    ...
    def test_power_profile_integration(self):
        ...
        for model_name, profile in power_profiles.items():
            with self.subTest(model=model_name):
                ...
                expected_energy_wh = power_w * profile['avg_inference_time_seconds'] / 3600
                final_energy = self.battery.get_level_wh()
                actual_energy_used = initial_energy - final_energy
