...
class TestIntegrationShort(unittest.TestCase):
    ...
    def validate_controller_comparison(self, results):
        ...
        # Extract key metrics
        completion_rates = {name: data['metrics']['task_completion_rate'] 
                          for name, data in results.items()}
        energy_usage = {name: data['metrics']['total_energy_mwh'] / 1000  # Convert mWh to Wh
                       for name, data in results.items()}
        clean_energy_pct = {name: data['metrics']['clean_energy_percentage'] 
                          for name, data in results.items()}
        ...
        for controller_name, rate in completion_rates.items():
            self.assertGreater(
                rate, 0, 
                f"{controller_name} should complete some tasks"
            )
        ...
        for accel in accelerations:
            with self.subTest(acceleration=accel):
                ...
                performance_results[accel] = {
                    'elapsed_time': elapsed_time,
                    'tasks_per_second': metrics['total_tasks'] / elapsed_time if elapsed_time > 0 else 0
                }
    ...
    def test_csv_export_integration(self):
        ...
        timeseries_file = exporter.export_detailed_timeseries(metrics, simulation_info)
